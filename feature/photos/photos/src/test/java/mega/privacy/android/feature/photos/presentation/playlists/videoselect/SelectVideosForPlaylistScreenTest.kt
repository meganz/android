...
import mega.privacy.android.feature.photos.components.SELECT_VIDEOS_BOTTOM_VIEW_ADD_BUTTON_TEST_TAG
import mega.privacy.android.feature.photos.components.SELECT_VIDEOS_BOTTOM_VIEW_CANCEL_BUTTON_TEST_TAG
import mega.privacy.android.feature.photos.components.SELECT_VIDEOS_BOTTOM_VIEW_ROW_TEST_TAG
...
class SelectVideosForPlaylistScreenTest {
    ...
    @Test
    fun `test that onBackPressed is invoked when cancel button in bottom view is clicked`() {
    ...
    @Test
    fun `test that confirmAddVideos is invoked when add button in bottom view is clicked and items are selected`() {
        val confirmAddVideos = mock<() -> Unit>()
        val items = listOf(
            SelectVideoItemUiEntity(
                id = NodeId(1L),
                name = "video.mp4",
        ...
        verify(confirmAddVideos).invoke()
    }
