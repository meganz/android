...
internal class MoveViewModelTest {
    ...
    fun `test that uiState emits Data with root and empty path when there is no last target`() =
        runTest(testDispatcher) {
            ...
            getMoveLatestTargetUseCase.stub { on { invoke() } doReturn null }
            initUnderTest()
    ...
    fun `test that uiState resolves the target path when the last target lives under root`() =
        runTest(testDispatcher) {
            ...
            getRootNodeIdUseCase.stub { on { invoke() } doReturn root }
            getMoveLatestTargetUseCase.stub { on { invoke() } doReturn target.longValue }
    ...
    fun `test that uiState returns empty path when the last target equals root`() =
        runTest(testDispatcher) {
            val root = NodeId(1L)
            getRootNodeIdUseCase.stub { on { invoke() } doReturn root }
            getMoveLatestTargetUseCase.stub { on { invoke() } doReturn root.longValue }
            initUnderTest()
