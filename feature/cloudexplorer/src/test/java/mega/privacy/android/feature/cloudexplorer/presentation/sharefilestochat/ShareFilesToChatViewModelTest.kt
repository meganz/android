...
class ShareFilesToChatViewModelTest {

    private lateinit var viewModel: ShareFilesToChatViewModel
    ...
    fun setUp() {
        ...
        viewModel = ShareFilesToChatViewModel(
    ...
    fun `test that ui state exposes root node id from use case`() = runTest(testDispatcher) {
        ...
        viewModel.uiState.test {
            var state: ShareFilesToChatUiState = awaitItem()
            if (state is ShareFilesToChatUiState.Loading) {
                state = awaitItem()
            }
            val data = state as ShareFilesToChatUiState.Data
