...
internal class ImportAlbumViewModelTest {

    private lateinit var underTest: ImportAlbumViewModel
    ...
    fun `test that uiState emits Data with root node id when use case succeeds`() =
        runTest(testDispatcher) {
            ...
            underTest = ImportAlbumViewModel(getRootNodeIdUseCase)

            underTest.uiState.test {
                var state: ImportAlbumUiState = awaitItem()
                if (state is ImportAlbumUiState.Loading) {
                    state = awaitItem()
                }
                assertThat(state).isEqualTo(ImportAlbumUiState.Data(rootNodeId = expectedRoot))
            }
        }
    ...
    fun `test that uiState falls back to NodeId minus one when use case returns null`() =
        runTest(testDispatcher) {
            ...
            underTest.uiState.test {
                ...
                assertThat(state).isEqualTo(ImportAlbumUiState.Data(rootNodeId = NodeId(-1)))
            }
        }
