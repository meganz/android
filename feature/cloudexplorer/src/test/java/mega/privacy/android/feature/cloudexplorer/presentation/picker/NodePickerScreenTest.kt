...
internal class NodePickerScreenTest {
    ...
    fun `test that nothing is rendered while loading`() {
        setContent(uiState = NodePickerUiState.Loading)
    ...
    fun `test that the explorer is rendered in the data state`() {
        setContent(uiState = NodePickerUiState.Data(rootNodeId = NodeId(1L)))

        composeTestRule.onNodeWithTag(CLOUD_EXPLORER_VIEW_TAG).assertIsDisplayed()
    }

    private fun setContent(uiState: NodePickerUiState) {
