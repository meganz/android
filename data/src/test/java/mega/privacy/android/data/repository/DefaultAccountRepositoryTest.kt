...
class DefaultAccountRepositoryTest {
    ...
    @Test
    fun `test that create account with public handle is successful when SDK returns success`() =
        runTest {
            val userEmail = "email"
            val userPassword = "password"
            val userSessionKey = "sessionKey"
            val userFirstName = "firstName"
            val userLastName = "lastName"
            val userLastPublicHandle = 123456L
            val userLastPublicHandleType = 1
            val userLastPublicHandleTimestamp = 123456L
            ...
            whenever(
                megaApiGateway.createAccount(
                    ...
                    lastAccessTimestamp = any(),
                    listener = any(),
                )
            ).thenAnswer {
                ((it.arguments[7]) as OptionalMegaRequestListenerInterface).onRequestFinish(
                    api = mock(),
                    request = megaRequest,
                    error = megaError,
                )
            ...
            val actualResult = underTest.createAccount(
                ...
                firstName = userFirstName,
                lastName = userLastName,
                lastPublicHandle = userLastPublicHandle,
                lastPublicHandleType = userLastPublicHandleType,
                lastPublicHandleTimeStamp = userLastPublicHandleTimestamp
            )
            assertThat(actualResult).isEqualTo(expectedResult)
        }

    @Test
    fun `test that create account with public handle throws AccountAlreadyExists Exception when SDK returns account exists error`() =
        runTest {
            val userEmail = "email"
            val userPassword = "password"
            val userFirstName = "firstName"
    ...
    @Test
    fun `test that create account with public handle throws Unknown Exception when SDK returns generic error`() =
        runTest {
            val userEmail = "email"
            val userPassword = "password"
            val userFirstName = "firstName"
