...
class CameraUploadsStatusInfoMapper @Inject constructor() {
     ...
     */
    operator fun invoke(progress: Data): CameraUploadsStatusInfo? {
        return when (progress.keyValueMap[STATUS_INFO]) {
            PROGRESS -> {
                with(progress) {
            ...
            FOLDER_UNAVAILABLE -> {
                val folderType = CameraUploadFolderType.values()[progress.getInt(
                    FOLDER_TYPE,
