...
internal class CopyNodeListUseCaseTest {
    private lateinit var underTest: CopyNodeListUseCase
    ...
    internal fun `test that node list size is returned`() = runTest {
        ...
        val nodes = List(5) { mock<MegaNode>() }
    ...
    internal fun `test that QuotaExceededMegaException errors are thrown`() = runTest {
        val parent = mock<MegaNode> { on { handle }.thenReturn(1L) }
        megaApi.stub {
            onBlocking { getMegaNodeByHandle(any()) }.thenReturn(parent)
            onBlocking { isForeignNode(any()) }.thenReturn(false)
        }
        stubCopyMethod(result = MegaError.API_EOVERQUOTA)

        val nodes = List(5) { mock<MegaNode>() }
        assertThrows<QuotaExceededMegaException> { underTest(nodes, 1L) }
    ...
    internal fun `test that cancellation stops execution`() = runTest {
        ...
        val nodes = List(5) { mock<MegaNode>() }
        launch { underTest(nodes, 1L) }.cancelAndJoin()
