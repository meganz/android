...
import mega.privacy.android.app.presentation.documentsection.view.DocumentGridView
...
class DocumentGridViewTest {
    ...
    fun `test that onClick is invoked when the item is clicked`() {
        ...
        composeTestRule.onNodeWithTag(DOCUMENT_SECTION_GRID_ITEM_VIEW_TEST_TAG + "0").performClick()
        verify(onClick).invoke(document1, 0)
        composeTestRule.onNodeWithTag(DOCUMENT_SECTION_GRID_ITEM_VIEW_TEST_TAG + "1").performClick()
        verify(onClick).invoke(document2, 1)
        composeTestRule.onNodeWithTag(DOCUMENT_SECTION_GRID_ITEM_VIEW_TEST_TAG + "2").performClick()
