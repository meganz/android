...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    public static synchronized DatabaseHandler getDbHandler(Context context) {
        ...
        if (instance == null) {
            ...
            LegacyLoggingSettings legacyLoggingSettings = EntryPointAccessors.fromApplication(
                    MegaApplication.getInstance(),
                    LegacyLoggingEntryPoint.class
            ).getLegacyLoggingSettings();
            instance = new DatabaseHandler(context, legacyLoggingSettings);
        }

        return instance;
    }

    public DatabaseHandler(Context context, LegacyLoggingSettings legacyLoggingSettings) {
        ...
        if (oldVersion <= 18) {
            ...
            for (int i = 0; i < offlinesOld.size(); i++) {
                ...
                if (offline.getType() == null || offline.getType().equals("0") || offline.getType().equals("1")) {
                    Timber.d("Not encrypted: %s", offline.getName());
                    this.setOfflineFile(offline, db);    //using the method that encrypts
                } else {
                    Timber.d("Encrypted: %s", offline.getName());
...
//			log("UPDATE_PREFERENCES_TABLE SYNC WIFI: " + UPDATE_PREFERENCES_TABLE);
    ...
    private MegaAttributes getAttributes(SQLiteDatabase db) {
        ...
        try (Cursor cursor = db.rawQuery(selectQuery, null)) {
            if (cursor != null && cursor.moveToFirst()) {
                ...
                if (!legacyLoggingSettings.areSDKLogsEnabled() && cursor.getColumnIndex(KEY_FILE_LOGGER_SDK) != INVALID_VALUE) {
                    String fileLoggerSDK = decrypt(cursor.getString(getColumnIndex(cursor, KEY_FILE_LOGGER_SDK)));
                    legacyLoggingSettings.updateSDKLogs(Boolean.parseBoolean(fileLoggerSDK));
                }
                ...
                if (!legacyLoggingSettings.areKarereLogsEnabled() && cursor.getColumnIndex(KEY_FILE_LOGGER_KARERE) != INVALID_VALUE) {
                    String fileLoggerKarere = decrypt(cursor.getString(getColumnIndex(cursor, KEY_FILE_LOGGER_KARERE)));
                    legacyLoggingSettings.updateKarereLogs(Boolean.parseBoolean(fileLoggerKarere));
                }
    ...
    public MegaContactDB findContactByHandle(String handle) {
        Timber.d("findContactByHandle: %s", handle);
    ...
    public long setOfflineFile(MegaOffline offline) {
        Timber.d("setOfflineFile: %s", offline.getHandle());
        ContentValues values = new ContentValues();
    ...
    public void setLastCloudFolder(String folderHandle) {
        ...
        try (Cursor cursor = db.rawQuery(selectQuery, null)) {
            if (cursor != null && cursor.moveToFirst()) {
                ...
                db.execSQL(UPDATE_PREFERENCES_TABLE);
                Timber.d("KEY_LAST_CLOUD_FOLDER_HANDLE UPLOAD FOLDER: %s", UPDATE_PREFERENCES_TABLE);
            } else {
