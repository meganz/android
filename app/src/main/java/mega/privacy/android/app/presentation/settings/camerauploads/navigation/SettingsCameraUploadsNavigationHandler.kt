...
/**
 * Simplified [NavigationHandler] for [SettingsCameraUploadsActivity].
 *
 * Hosts its own [PendingBackStack] so cloud explorer destinations can be reached without
 * going through the main app shell. Auth/passcode concerns are handled by the activity's
 * surrounding containers, so the handler does not gate them here.
 */
class SettingsCameraUploadsNavigationHandler(
