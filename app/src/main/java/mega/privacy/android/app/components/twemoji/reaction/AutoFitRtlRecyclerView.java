...
public class AutoFitRtlRecyclerView extends RecyclerView {
    private GridLayoutManager manager;
    private int columnWidth = INVALID_POSITION;

    public AutoFitRtlRecyclerView(Context context) {
        super(context);
        initialization(context, null);
    }

    public AutoFitRtlRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialization(context, attrs);
    }

    public AutoFitRtlRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initialization(Context context, AttributeSet attrs) {
        try{
            ...
            manager = new RtlGridLayoutManager(context, 4, RecyclerView.VERTICAL, false);
