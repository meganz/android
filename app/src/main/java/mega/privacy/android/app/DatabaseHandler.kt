...
import mega.privacy.android.app.sync.camerauploads.CameraUploadSyncManager.updatePrimaryTargetNode
import mega.privacy.android.app.sync.camerauploads.CameraUploadSyncManager.updateSecondaryTargetNode
import mega.privacy.android.app.sync.deleteSQL
import mega.privacy.android.app.sync.updateSQL
...
open class DatabaseHandler(
    ...
    fun batchInsertMegaContacts(contacts: List<MegaContact>?) {
    ...
    val megaContacts: ArrayList<MegaContact>
        get() {
    ...
    private fun setAttributes(db: SQLiteDatabase, attr: MegaAttributes?) {
        ...
        values.put(KEY_PAYMENT_METHODS_TIMESTAMP, encrypt(attr.paymentMethodsTimeStamp))
        ...
        try {
            db.rawQuery(selectQuery, null)?.use { cursor ->
                if (cursor.moveToFirst()) {
                    ...
                    val paymentMethodsTimeStamp = decrypt(cursor.getString(getColumnIndex(cursor,
                        KEY_PAYMENT_METHODS_TIMESTAMP)))
    ...
    fun setCamSyncHandle(handle: Long) {
        ...
        try {
            db.rawQuery(selectQuery, null).use { cursor ->
                ...
                updatePrimaryTargetNode(handle)
            }
        } catch (e: Exception) {
            Timber.e(e, "Exception opening or managing DB cursor")
        }
    ...
    fun setSecondaryFolderHandle(handle: Long) {
        ...
        try {
            db.rawQuery(selectQuery, null).use { cursor ->
                ...
                updateSecondaryTargetNode(handle)
            }
        } catch (e: Exception) {
            Timber.e(e, "Exception opening or managing DB cursor")
        }
    ...
    fun setPaymentMethodsTimeStamp() {
        Timber.d("setPaymentMethodsTimeStamp")
        val paymentMethodsTimeStamp = System.currentTimeMillis() / 1000
        val selectQuery = "SELECT * FROM $TABLE_ATTRIBUTES"
        val values = ContentValues()
        try {
            db.rawQuery(selectQuery, null).use { cursor ->
                if (cursor != null && cursor.moveToFirst()) {
                    val UPDATE_ATTRIBUTE_TABLE =
                        "UPDATE $TABLE_ATTRIBUTES SET $KEY_PAYMENT_METHODS_TIMESTAMP= '${
                            encrypt(paymentMethodsTimeStamp.toString())
                        }' WHERE $KEY_ID = '1'"
                    db.execSQL(UPDATE_ATTRIBUTE_TABLE)
                } else {
                    values.put(KEY_PAYMENT_METHODS_TIMESTAMP,
                        encrypt(paymentMethodsTimeStamp.toString()))
                    db.insert(TABLE_ATTRIBUTES, null, values)
                }
            }
    ...
    fun addPendingMessage(
        ...
        state: Int = PendingMessageSingle.STATE_PREPARING,
    ): Long {
        val values = ContentValues()
    ...
    companion object {
        ...
        const val KEY_BACKUP_TARGET_NODE = "target_node"
        const val KEY_BACKUP_LOCAL_FOLDER = "local_folder"
        const val KEY_BACKUP_NAME = "backup_name"
        const val KEY_BACKUP_STATE = "state"
        const val KEY_BACKUP_SUB_STATE = "sub_state"
        const val KEY_BACKUP_EXTRA_DATA = "extra_data"
        const val KEY_BACKUP_START_TIME = "start_timestamp"
        const val KEY_BACKUP_LAST_TIME = "last_sync_timestamp"
        const val KEY_BACKUP_TARGET_NODE_PATH = "target_folder_path"
        const val KEY_BACKUP_EX = "exclude_subolders"
        const val KEY_BACKUP_DEL = "delete_empty_subolders"
        const val KEY_BACKUP_OUTDATED = "outdated"
