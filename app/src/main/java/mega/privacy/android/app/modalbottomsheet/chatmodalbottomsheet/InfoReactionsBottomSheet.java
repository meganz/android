...
import mega.privacy.android.app.components.reaction.CustomViewPager;
...
public class InfoReactionsBottomSheet extends ViewPagerBottomSheetDialogFragment implements ViewPager.OnPageChangeListener {
    ...
    private RelativeLayout[] reactionTabs = null;
    ...
    public void setupDialog(final Dialog dialog, int style) {
        ...
        ArrayList<String> list = getReactionsList(listReactions, false);
        reactionTabs = new RelativeLayout[listReactions.size()];
        int indexToStart = 0 ;
        for (int i = 0; i < list.size(); i++) {
            reactionTabs[i] = inflateButton(context, list.get(i), infoReactionsTab);
        ...
        handleOnClicks(infoReactionsPager);

        if(reactionsPageAdapter == null) {
            reactionsPageAdapter = new InfoReactionPagerAdapter(context, list.size());
        }
    ...
    private RelativeLayout inflateButton(final Context context, String reaction, final ViewGroup parent) {
        ...
        reactionImage.setEmoji(emojis.get(0).emoji);
        parent.addView(button);
        return button;
    }
    private void handleOnClicks(final ViewPager reactionPager) {
        int totalLength = reactionTabs.length - 1;
        for (int i = 0; i < totalLength; i++) {
            reactionTabs[i].setOnClickListener(new ReactionsTabsClickListener(reactionPager, i));
        }
    }
    ...
    @Override public void onPageSelected(final int i) {
        if (reactionTabLastSelectedIndex != i) {
            ...
            if (reactionTabLastSelectedIndex >= 0 && reactionTabLastSelectedIndex < reactionTabs.length) {
                reactionTabs[reactionTabLastSelectedIndex].setSelected(false);
                if(reactionTabs[reactionTabLastSelectedIndex].getChildCount() > 1){
                    reactionTabs[reactionTabLastSelectedIndex].removeView(separator);
                }
            }

            reactionTabs[i].setSelected(true);
            if(reactionTabs[i].getChildCount() == 1) {
                RelativeLayout.LayoutParams lp = new RelativeLayout.LayoutParams(px2dp(40, outMetrics), px2dp(2, outMetrics));
                lp.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                reactionTabs[i].addView(separator, lp);
            }
    ...
    static class ReactionsTabsClickListener implements View.OnClickListener {
