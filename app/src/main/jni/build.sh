...
MEGA=mega
MEGA_JAVA_OUTPUT_PATH=${JNI_PATH}/../java
...
function createMEGABindings
{
    ...
    make -C ${MEGA} -f MakefileBindings ${MEGA_CONFIGURED} JAVA_BASE_OUTPUT_PATH=${MEGA_JAVA_OUTPUT_PATH} &> ${LOG_FILE}
}

function createMEGAchatBindings
{
    echo "* Creating MEGAchat Java bindings"
    swig -Imegachat/karere-native/src/ -c++ -java -package nz.mega.chat -outdir ../../main/java/nz/mega/chat/ -o megachat/megachat.cpp megachat/megachatapi.i
}
...
if [ "$1" == "clean" ]; then
    ...
    make -C ${MEGA} -f MakefileBindings clean JAVA_BASE_OUTPUT_PATH=${MEGA_JAVA_OUTPUT_PATH} &> ${LOG_FILE}
...
git clone --recursive -b feature/mega-chat-api https://code.developers.mega.co.nz/messenger/karere-native.git megachat/karere-native
else
echo "* MEGAchat source code already available"
fi

if [ ! -f megachat/megachat.cpp ]; then
createMEGAchatBindings
else
echo "* MEGAchat bindings already available"
fi
echo "* MEGAchat is ready"

echo "* Setting up MEGA"
if [ ! -f mega/java_wrap.cxx ]; then
createMEGABindings
else
echo "* MEGA bindings already available"
fi
echo "* MEGA is ready"
