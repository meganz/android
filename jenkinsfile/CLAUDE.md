...
The SDK lives at `sdk/src/main/jni/mega/sdk` and MEGAchat at `sdk/src/main/jni/megachat/sdk`. Both are git submodules. Prebuilt-SDK publishing is handled by the separate `build sdk-aar` command (SDK repo / its own Jenkins job), not by these pipelines.
