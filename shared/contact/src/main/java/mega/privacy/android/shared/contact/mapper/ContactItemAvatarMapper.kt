...
/**
 * Maps a domain [ContactItem] to the presentational [AvatarData] consumed by
 * `ContactItemView`.
 *
 * Produces an [AvatarData.Image] when the contact has an avatar file, otherwise
 * an [AvatarData.Initials] built from the contact's first letter and default
 * avatar color.
 */
class ContactItemAvatarMapper @Inject constructor() {
     ...
     */
    operator fun invoke(contactItem: ContactItem): AvatarData {
