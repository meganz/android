...
class NodeSourceTypeToViewTypeMapperTest {
    private val underTest = NodeSourceTypeToViewTypeMapper()

    @ParameterizedTest(name = "NodeSourceType {0} maps to {1}")
    @MethodSource("provideNodeSourceTypeMappings")
    fun `test that nodeSource type maps to expected view type`(
    ...
    private fun provideNodeSourceTypeMappings() = Stream.of(
