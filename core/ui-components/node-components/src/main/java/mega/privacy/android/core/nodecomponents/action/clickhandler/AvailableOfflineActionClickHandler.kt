...
class AvailableOfflineActionClickHandler @Inject constructor() : SingleNodeAction, MultiNodeAction {
    ...
    override fun handle(action: MenuAction, node: TypedNode, provider: SingleNodeActionProvider) {
        provider.viewModel.downloadNodesForOffline(nodes = listOf(node), withStartMessage = false)
    }
    ...
    override fun handle(
        ...
        provider.viewModel.downloadNodesForOffline(nodes = nodes, withStartMessage = false)
    }
}
