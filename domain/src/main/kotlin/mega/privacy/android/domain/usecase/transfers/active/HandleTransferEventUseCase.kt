...
class HandleTransferEventUseCase @Inject internal constructor(
    ...
    override suspend operator fun invoke(vararg events: TransferEvent) {
        events.asList().takeIf { it.isNotEmpty() }?.let { transferEvents ->
            ...
            coroutineScope {
                ...
                val checkCompletedTransferDeferred =
                    async { checkCompletedTransfers(transferEvents) }
                ...
                checkCompletedTransferDeferred.await()
            }
        }
    }
    ...
    private suspend fun checkCompletedTransfers(events: List<TransferEvent>) {
        ...
        }.takeIf { it.isNotEmpty() }?.let { manualTransferEvents ->
