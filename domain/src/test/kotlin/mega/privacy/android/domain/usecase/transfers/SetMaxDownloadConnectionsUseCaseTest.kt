...
class SetMaxDownloadConnectionsUseCaseTest {
    private lateinit var underTest: SetMaxDownloadConnectionsUseCase

    private val transferRepository = mock<TransferRepository>()

    @BeforeAll
    fun setUp() {
        underTest = SetMaxDownloadConnectionsUseCase(
    ...
    fun `test that setMaxDownloadConnections in the repository is invoked when value is in range`(
        connections: Int,
    ) = runTest {
        underTest(connections)
        verify(transferRepository).setMaxDownloadConnections(connections)
    }
