...
class GetVideoNodesFromPublicLinksUseCaseTest {
    lateinit var underTest: GetVideoNodesFromPublicLinksUseCase
    ...
    @BeforeAll
    fun setUp() {
        underTest = GetVideoNodesFromPublicLinksUseCase(
    ...
    fun `test that videos is not empty`() = runTest {
        ...
        whenever(mediaPlayerRepository.getVideoNodesFromPublicLinks(order)).thenReturn(list)
    ...
    fun `test that videos is empty`() = runTest {
        whenever(mediaPlayerRepository.getVideoNodesFromPublicLinks(order)).thenReturn(emptyList())
