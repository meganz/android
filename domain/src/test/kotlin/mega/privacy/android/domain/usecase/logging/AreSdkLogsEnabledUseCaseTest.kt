...
class AreSdkLogsEnabledUseCaseTest {

    private lateinit var underTest: AreSdkLogsEnabledUseCase

    private val repository: LoggingRepository = mock()

    @BeforeEach
    fun setUp() {
        underTest = AreSdkLogsEnabledUseCase(
    ...
    fun `test that the correct boolean value is returned`(value: Boolean) = runTest {
        whenever(repository.isSdkLoggingEnabled()) doReturn flowOf(value)
