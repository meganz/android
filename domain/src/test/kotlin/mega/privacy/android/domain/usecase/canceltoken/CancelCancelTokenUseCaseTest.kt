...
class CancelCancelTokenUseCaseTest {

    private lateinit var underTest: CancelCancelTokenUseCase
    ...
    @BeforeAll
    fun setup() {
        underTest = CancelCancelTokenUseCase(
    ...
    fun `test that repository's cancel token method is called use case is invoked`() =
        runTest {
            underTest()
            verify(cancelTokenRepository).cancelCurrentToken()
        }
}