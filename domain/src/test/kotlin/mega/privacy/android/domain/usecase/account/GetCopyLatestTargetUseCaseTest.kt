...
class GetCopyLatestTargetUseCaseTest {
    ...
    fun `test that invoke returns the path when it is not in the rubbish or deleted`() = runTest {
        val path = 1234L
        whenever(accountRepository.getLatestTargetCopyPreference()).thenReturn(path)
    ...
    fun `test that invoke returns null when the repository returns null`() = runTest {
        whenever(accountRepository.getLatestTargetCopyPreference()).thenReturn(null)
        assertThat(underTest()).isNull()
