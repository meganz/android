...
class GetLinksSortOrderUseCaseTest {
    ...
    @Test
    fun `test that default value is returned when repository returns null`() = runTest {
        val expected = SortOrder.ORDER_DEFAULT_ASC
        whenever(sortOrderRepository.getLinksSortOrder()).thenReturn(null)
        assertThat(underTest()).isEqualTo(expected)
    }

    @Test
    fun `test that returned value matches when repository returns non null value`() = runTest {
        val expected = SortOrder.ORDER_CREATION_ASC
        whenever(sortOrderRepository.getLinksSortOrder()).thenReturn(expected)
        assertThat(underTest()).isEqualTo(expected)
    }
}
