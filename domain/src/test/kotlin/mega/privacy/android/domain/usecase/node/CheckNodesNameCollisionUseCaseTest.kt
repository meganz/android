...
internal class CheckNodesNameCollisionUseCaseTest {
    ...
    @Test
    fun `test that a collision is reported for a copy when the node already lives in the target folder`() =
        runTest {
            val nodes = mapOf(2L to 101L)
            val currentNode = mock<FileNode> {
                on { id }.thenReturn(NodeId(2L))
            ...
            whenever(repository.getInvalidHandle()).thenReturn(INVALID_NODE_HANDLE)
            whenever(getNodeByHandleUseCase(101L, false)).thenReturn(parent)
            whenever(isNodeInRubbishBinUseCase(NodeId(any()))).thenReturn(false)
            whenever(getNodeByHandleUseCase(2L, true)).thenReturn(currentNode)
            whenever(getChildNodeUseCase(NodeId(101L), currentNode.name)).thenReturn(currentNode)

            assertThat(underTest(nodes, NodeNameCollisionType.COPY)).isEqualTo(
                NodeNameCollisionsResult(
                    emptyMap(),
                    mapOf(
                        2L to NodeNameCollision.Default(
                            ...
                            name = currentNode.name,
                            size = currentNode.size,
                            childFolderCount = parent.childFolderCount,
                            childFileCount = parent.childFileCount,
                            lastModified = currentNode.modificationTime,
                            isFile = true,
