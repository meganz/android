...
class DefaultGetOthersSortOrderTest {
    lateinit var underTest: GetOthersSortOrder
    private val sortOrderRepository = mock<SortOrderRepository>()

    @Before
    fun setUp() {
        underTest = DefaultGetOthersSortOrder(sortOrderRepository)
    }
    ...
    fun `test that default value is returned when repository returns null`() {
        runTest {
            ...
            whenever(sortOrderRepository.getOthersSortOrder()).thenReturn(
                null
            )
    ...
    fun `test that returned value matches when repository returns non null value`() {
        runTest {
            val expected = SortOrder.ORDER_LABEL_ASC
            whenever(sortOrderRepository.getOthersSortOrder()).thenReturn(
                expected
            )
            assertThat(underTest()).isEqualTo(expected)
