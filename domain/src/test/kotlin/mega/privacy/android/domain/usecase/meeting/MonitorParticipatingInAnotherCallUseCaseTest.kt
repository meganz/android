...
internal class MonitorParticipatingInAnotherCallUseCaseTest {

    private lateinit var underTest: MonitorParticipatingInAnotherCallUseCase
    ...
    @BeforeAll
    fun setup() {
        underTest = MonitorParticipatingInAnotherCallUseCase(
    ...
    fun `test that user has a call in other chat when`(
        ...
        whenever(getCallIdsOfOthersCallsUseCase(currentChatId))
        ...
        underTest.invoke(currentChatId).test {
            Truth.assertThat(awaitItem()).isEqualTo(expectedInitialList.toList())
            updateFlow.emit(updatedCall)
            advanceUntilIdle()
            Truth.assertThat(awaitItem()).isEqualTo(expectedUpdatedList.toList())
        }
    }

    private fun ChatCallStatus.isCurrentCall() =
        this != ChatCallStatus.WaitingRoom
                && this != ChatCallStatus.UserNoPresent
