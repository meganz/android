...
class CalculateCurrencyAmountUseCaseTest {
    private lateinit var underTest: CalculateCurrencyAmountUseCase
    ...
    @Before
    fun setUp() {
        underTest = CalculateCurrencyAmountUseCase()
    }

    @Test
    fun `test that CalculateCurrencyAmountUseCase return Currency Amount  correctly from SystemCurrencyPoint`() {
        assertEquals(currencyAmountResult, underTest.invoke(systemCurrencyPoint, currency))
    }

    @Test
    fun `test that CalculateCurrencyAmountUseCase return Currency Amount  correctly from LocalCurrencyPoint`() {
