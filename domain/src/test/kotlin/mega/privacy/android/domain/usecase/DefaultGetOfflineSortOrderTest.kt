...
class DefaultGetOfflineSortOrderTest {
    lateinit var underTest: GetOfflineSortOrder
    private val sortOrderRepository = mock<SortOrderRepository>()

    @Before
    fun setUp() {
        underTest = DefaultGetOfflineSortOrder(sortOrderRepository)
    }
    ...
    fun `test that default value is returned when repository returns null`() {
        runTest {
            ...
            whenever(sortOrderRepository.getOfflineSortOrder()).thenReturn(
                null
            )
    ...
    fun `test that returned value matches when repository returns non null value`() {
        runTest {
            val expected = SortOrder.ORDER_MODIFICATION_ASC
            whenever(sortOrderRepository.getOfflineSortOrder()).thenReturn(
                expected
            )
            assertThat(underTest()).isEqualTo(expected)
