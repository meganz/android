...
class GetVideoNodesUseCaseTest {
    ...
    @BeforeAll
    fun setUp() {
        underTest = GetVideoNodesUseCase(
    ...
    fun `test that videos is not empty`() = runTest {
        ...
        whenever(mediaPlayerRepository.getVideoNodes(order)).thenReturn(list)
    ...
    fun `test that videos is empty`() = runTest {
        whenever(mediaPlayerRepository.getVideoNodes(order)).thenReturn(emptyList())
