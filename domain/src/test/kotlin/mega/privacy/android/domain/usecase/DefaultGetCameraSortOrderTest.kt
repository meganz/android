...
class DefaultGetCameraSortOrderTest {
    lateinit var underTest: GetCameraSortOrder
    private val sortOrderRepository = mock<SortOrderRepository>()

    @Before
    fun setUp() {
        underTest = DefaultGetCameraSortOrder(sortOrderRepository)
    }
    ...
    fun `test that default value is returned when repository returns null`() {
        runTest {
            ...
            whenever(sortOrderRepository.getCameraSortOrder()).thenReturn(
                null
            )
