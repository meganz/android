...
class FilterCloudDriveMediaUseCaseTest {

    private lateinit var underTest: FilterCloudDriveMediaUseCase
    ...
    fun setUp() = runTest {
        ...
        underTest = FilterCloudDriveMediaUseCase(photosRepository = photosRepository)
    }
    ...
    fun `test that non filtered values are returned`() = runTest {
        ...
        val expectedPhoto = mock<TypedFileNode> {
            on { parentId } doReturn NodeId(longValue = 3L)
        }
        ...
        assertThat(actual).containsExactly(expectedPhoto)
    }
