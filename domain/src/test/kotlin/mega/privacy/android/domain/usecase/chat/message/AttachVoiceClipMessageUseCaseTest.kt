...
class AttachVoiceClipMessageUseCaseTest {

    private lateinit var underTest: AttachVoiceClipMessageUseCase
    ...
    @BeforeEach
    fun setup() {
        underTest = AttachVoiceClipMessageUseCase(
            chatRepository,
    ...
    fun `test that message is attached`() =
        runTest {
            whenever(chatMessageRepository.attachVoiceMessage(chatId, nodeHandle)).thenReturn(msgId)
