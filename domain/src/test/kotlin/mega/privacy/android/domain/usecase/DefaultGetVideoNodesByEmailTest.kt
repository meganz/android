...
class DefaultGetVideoNodesByEmailTest {
    lateinit var underTest: GetVideoNodesByEmail
    ...
    @Before
    fun setUp() {
        underTest = DefaultGetVideoNodesByEmail(mediaPlayerRepository, addNodeType)
    }
    ...
    fun `test that the AddNodeType has been invoked`() =
        runTest {
            ...
            whenever(mediaPlayerRepository.getVideoNodesByEmail(email)).thenReturn(unTypedNodeList)
