...
class StartUploadingAllPendingMessagesUseCaseTest {
    ...
    @Test
    fun `test that pending message is set to compressing state and new path when file needs compression`() =
        runTest {
            val pendingMessage = stubPendingMessage()
    ...
    @Test
    fun `test that pending message starts uploading when file does not needs compression`() =
        runTest {
            val pendingMessage = stubPendingMessage()
            val file = File("video.mp4")
    ...
    @Test
    fun `test that multiple pending messages start uploading when file does not needs compression`() =
        runTest {
