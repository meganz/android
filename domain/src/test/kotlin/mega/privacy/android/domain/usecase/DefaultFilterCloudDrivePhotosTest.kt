...
class DefaultFilterCloudDrivePhotosTest {
    private lateinit var underTest: FilterCloudDrivePhotos
    ...
    @Before
    fun setUp() {
        underTest = DefaultFilterCloudDrivePhotos(photosRepository = photosRepository)
    }

    @Test
    fun `test that photos in camera upload folder are filtered out`() = runTest {
    ...
    @Test
    fun `test that photos in media upload folder are filtered out`() = runTest {
        val photo = mock<Photo> {
    ...
    @Test
    fun `test that photos in neither folder are returned`() = runTest {
        val photo = mock<Photo> {
    ...
    fun `test that non filtered values are returned`() = runTest {
        ...
        val expectedPhoto = mock<Photo> {
            on { parentId }.thenReturn(3L)
        }
        assertThat(underTest(listOf(mediaUploadPhoto,
            cameraUploadPhoto,
            expectedPhoto))).containsExactly(expectedPhoto)
    }
